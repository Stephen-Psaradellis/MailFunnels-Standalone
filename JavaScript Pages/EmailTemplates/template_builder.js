$(function() {
    ...
    var send_test_email_button = $('#send_test_email_button');
    ...
    set_default_button.on('click', function() {
        $.ajax({
            type:'POST',
            url: '/ajax/template/set_default_html',
            dataType: "json",
            data: {
                html: $('#mf-template-builder').data('contentbuilder').html()
            },
            error: function(e) {
                console.log(e);
            },
    ...
    template_settings_submit.on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#template_builder_name').html(template_name_val);
            }
        });
    ...
    template_clear_button.on('click', function(){
        $("#mf-template-builder").data('contentbuilder').loadHTML("");
    ...
    function selectAsset(assetValue) {
        ...
        $('#mf-template-builder').find("img").attr('src', assetValue);
    ...
    function init() {
        ...
        $("#mf-template-builder").contentbuilder({
            snippetCustomCode: true,
            absolutePath: true,
            snippetFile: '../builder_assets/template_builder/assets/minimalist-basic/snippets.html',
            toolbar: 'left',
        ...
        $("#mf-template-builder").data('contentbuilder').loadHTML(current_html);
